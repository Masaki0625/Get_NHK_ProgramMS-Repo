...
public class GetNHKProgramNowOnAirLogic {
	...
	public Response Service(RequestEntity requestEntity, RequestBodyEntity requestBodyEntity) {
		...
		response = GetNHKNowOnAir(requestEntity, requestBodyEntity, apiKey);
		if(response.getStatus() != Response.Status.OK.getStatusCode()) {
	 ...
	 */
	private Response GetNHKNowOnAir(RequestEntity requestEntity, RequestBodyEntity requestBodyEntity, StringBuilder apiKey) {
		long startTime = System.currentTimeMillis();
